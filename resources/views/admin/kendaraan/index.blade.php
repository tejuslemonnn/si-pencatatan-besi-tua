        ...
        <div class="overflow-x-auto">
            <table id="example" class="table table-bordered display nowrap">
                ...
                <tbody>
                    ...
                    @endif --}}
                            <td>
                                <a href="{{ route('kendaraan.show', ['kendaraan' => $row->id]) }}" class="btn btn-info"><i
                                        class="fas fa-eye"></i>
                                    Detail</a>

                                @if (auth()->user()->role == 'admin_perusahaan' && $row->status != 1)
                                    <a href="{{ route('kendaraan.edit', ['kendaraan' => $row->id]) }}"
                                        class="btn btn-warning"><i class="fas fa-edit"></i> Ubah
                                    </a>
                                    <form action="{{ route('kendaraan.destroy', ['kendaraan' => $row->id]) }}"
